...
const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    type: 'case-study',
    problem:
      'Users needed a highly scalable way to map digital wardrobes to localized weather patterns in real-time.',
    solution:
      'Engineered a decoupled backend system that ingests weather APIs and computes combinatorial clothing logic.',
  ...
  },
  {
    ...
    type: 'case-study',
    problem:
      'Content aggregators suffered from blocking IO and slow client-side rendering under heavy payload.',
    solution:
      'Built a fully SSR-capable aggregation engine that pre-fetches and caches global news nodes.',
  ...
  },
  {
    ...
    type: 'case-study',
    problem:
      'Monolithic e-commerce platforms were too rigid for rapid feature iteration and complex state changes.',
    solution:
      'Architected a headless commerce frontend with strict component boundaries and Redux-like state.',
...
const Projects: React.FC = () => {
  const [selectedRepo, setSelectedRepo] = useState<CaseStudy | null>(null);
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 min-h-[600px] relative">
          ...
          <div
            className={`flex flex-col gap-4 transition-all duration-500 ease-in-out ${
              selectedRepo && !isMobile ? 'lg:w-1/3' : 'w-full lg:w-1/2'
            }`}
          >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`terminal-label text-[9px] tracking-[0.2em] ${
                            isSelected ? 'text-accent-primary' : 'text-zinc-600'
                          }`}
                        >
                          {repo.id.toUpperCase()}
                        </span>
                      ...
                      <h3
                        className={`font-bold tracking-tight truncate transition-colors duration-300 ${
                          isSelected ? 'text-white text-xl' : 'text-zinc-300 text-lg'
                        }`}
                      >
                        {repo.name}
                      </h3>
                      <div className="flex flex-wrap gap-2 mt-4">
                        {repo.stack.slice(0, 3).map((s) => (
                          <span
                            key={s}
                            className="px-2 py-0.5 bg-white/5 rounded text-[9px] font-mono text-zinc-400"
                          >
                            {s}
                          </span>
                        ))}
          ...
          {/* Right Side: Case Study Detail Panel */}
          <AnimatePresence mode="wait">
            {selectedRepo && (
              <motion.div
                ...
                className={`fixed inset-4 z-50 lg:static lg:inset-auto lg:z-auto ${
                  isMobile ? 'flex items-center justify-center' : 'flex-1 lg:w-2/3 lg:block'
                }`}
              >
                {/* Mobile Backdrop */}
                {isMobile && (
                  ...
                  <div className="p-8 border-b border-white/5 bg-white/[0.01] flex justify-between items-start gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {selectedRepo.stack.map((s) => (
                          <span
                            key={s}
                            className="px-2 py-1 bg-accent-primary/10 text-accent-primary rounded-md text-[10px] font-mono uppercase tracking-widest border border-accent-primary/20"
                          >
                            {s}
                          </span>
                        ))}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <a
                        ...
                        className="p-3 border border-white/5 hover:border-accent-primary/40 hover:text-accent-primary hover:bg-accent-primary/10 transition-all rounded-xl"
                      >
                        <Github size={18} />
                      </a>
                      {selectedRepo.demo !== '#' && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                          ...
                          <p className="text-xs text-zinc-300">
                            {selectedRepo.architecture.frontend}
                          </p>
                        </div>
                        <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                          <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs text-zinc-300">
                            {selectedRepo.architecture.backend}
                          </p>
                        </div>
                        <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                          <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs text-zinc-300">
                            {selectedRepo.architecture.database}
                          </p>
                        </div>
